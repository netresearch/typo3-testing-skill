...
2. **A coverage flag that never reaches PHPUnit.** `composer ci:test:php:functional -- --coverage-clover=cov.xml` silently produces no file if the script is wrapped in `sh -c '…phpunit…'` — composer appends the args *after* the quoted string, so they become `$0`/`$@` of the wrapper, not PHPUnit flags. Forward them: `sh -c '… phpunit … "$@"' --`. (One extension uploaded no coverage for three months this way.)
